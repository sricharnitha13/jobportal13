...
import { Sparkles, MapPin, Building2, Briefcase, ArrowLeft } from "lucide-react";
...
function RecommendedJobsPage() {
  ...
  const fetchRecommendedJobs = useCallback(async () => {
    try {
      const res = await fetch("https://jobportal13.onrender.com/jobs/recommended", {
  ...
  return (
    <div className="p-4 p-md-5" style={{ background: "#f5f7fb", minHeight: "100vh" }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-5 flex-wrap gap-3">
        <div>
          <h1 className="fw-bold mb-2" style={{ fontSize: "42px" }}>AI Recommended Jobs</h1>
          <p className="text-muted mb-0" style={{ fontSize: "18px" }}>
            Personalized opportunities based on your skills, interests, and profile.
          </p>
        </div>
        <button
      ...
      <div className="row">
        {jobs.length === 0 ? (
          <div className="col-12">
            <div className="bg-white shadow-sm p-5 text-center" style={{ borderRadius: "24px" }}>
              <Sparkles size={48} color="#6c63ff" />
              <h4 className="fw-bold mt-4">No Recommendations Yet</h4>
              <p className="text-muted mt-2">
                Update your profile skills to receive personalized AI job recommendations.
              </p>
            </div>
          </div>
        ) : (
          jobs.map((job) => (
            <div className="col-lg-6 mb-4" key={job.id}>
              <div className="bg-white shadow-sm p-4 h-100" style={{ borderRadius: "24px" }}>
                ...
                <p className="text-muted" style={{ minHeight: "70px" }}>{job.description}</p>

                <div className="mt-4">
                  <small className="fw-semibold text-muted">Required Skills</small>
                  <div className="d-flex flex-wrap gap-2 mt-2">
                    {job.requiredSkills?.split(",").map((skill, i) => (
                      <span key={i} className="badge bg-light text-dark px-3 py-2" style={{ borderRadius: "12px" }}>
                        {skill.trim()}
                      </span>
                    ))}
                  </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "16px",
                    fontWeight: "600",
                    background: "linear-gradient(to right, #5b5df0, #6c63ff)",
                    border: "none"
                  }}
                >
